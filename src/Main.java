...
public class Main {
		 ...
		 *   - Nunca acepta letras donde se espera número
		 *   - Nunca acepta números fuera del rango del menú
		 *   - Nunca acepta nombre vacío
		 *   - Todos los try-catch evitan que el juego crashee
		 *   - Si el audio falla, el juego sigue normalmente
		 */
		    ...
		    static void menuPrincipal() {
		        ...
		        while (!salir) {
		            ...
		            switch (opcion) {
		                ...
		                case 2: mostrarLore();       break;
		                case 3: mostrarCreditos();   break;
		                case 4: opcionesAudio();     break;
		                case 5:
		    ...
		    static Personaje seleccionarPersonaje(String nombre) {
		        ...
		        while (!seleccionValida) {
		            ...
		            switch (opcion) {
		                case 1: jugador = new Princesa(nombre);   seleccionValida = true; break;
		                case 2: jugador = new Caballero(nombre);  seleccionValida = true; break;
		                case 3: jugador = new Hechicero(nombre);  seleccionValida = true; break;
		            }
		        }

		        System.out.println("\n  Has elegido: " + jugador.getNombre()
		                         + " — " + jugador.getRol());
		        pausar();
		        return jugador;
		    }
		    ...
		    static void opcionesAudio() {
		        ...
		        while (!salir) {
		            ...
		            switch (op) {
		                case 1: Musica.toggleMusica();   break;
		                case 2: Musica.toggleEfectos();  break;
		                case 3: Musica.setVolumen(0.9f); System.out.println("  Volumen alto."); break;
		                case 4: Musica.setVolumen(0.4f); System.out.println("  Volumen bajo."); break;
		    ...
		    /**
		     * Lee un entero entre min y max.
		     * Repite el mensaje si:
		     *   - El usuario escribe letras
		     *   - El número está fuera del rango
		     *   - El usuario presiona Enter sin escribir nada
		     *   - Ocurre cualquier otro error de lectura
		     */
		        ...
		        while (true) {
		            try {
		                ...
		                if (linea == null || linea.trim().isEmpty()) {
		                    System.out.print("  ⚠ Debes ingresar un número entre "
		                                   + min + " y " + max + ": ");
		                    continue;
		                }
		                ...
		                if (valor >= min && valor <= max) {
		                    ...
		                    System.out.print("  ⚠ Opción inválida. Elige entre "
		                                   + min + " y " + max + ": ");
		                }

		            } catch (NumberFormatException e) {
		                System.out.print("  ⚠ Eso no es un número. Ingresa entre "
		                               + min + " y " + max + ": ");
		            } catch (Exception e) {
